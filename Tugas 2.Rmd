...
data1 <- import("https://github.com/kamaldims/MPDW/raw/refs/heads/main/Kurs%20Jual%20Rataan%20USD%20Fix.xlsx")
...
```{r}
data1.ts <- ts(data1$`Kurs Jual Rataan`)
```
...
# bagi data
training_ma <- data1[313:(313+n_train), ]
testing_ma  <- data1[(314+n_train):415, ]
...
Data pemulusan pada `Minggu Ke-t` selanjutnya digunakan sebagai nilai peramalan pada `Minggu Ke-t+1` sehingga hasil peramalan 1 `Minggu Ke-` kedepan adalah sebagai berikut.

```{r}
data.ramal<-c(NA,data.sma)
data.ramal #forecast 1 `Minggu Ke-t` ke depan
```

Selanjutnya dilakukan peramalan sebanyak 24 \`Minggu Ke-t\` sesuai dengan jumlah data uji. Pada metode SMA, seluruh hasil peramalan untuk 24 \`Minggu Ke-t\` ke depan akan memiliki nilai yang sama dengan hasil ramalan satu \`Minggu Ke-t\` ke depan.
...
data.gab #forecast 24 `Minggu Ke-t` ke depan
```

Adapun plot data deret waktu dari hasil peramalan yang dilakukan adalah sebagai berikut.

```{r}
ts.plot(data2, xlab="Time `Minggu Ke-` ", ylab="`Kurs Jual Rataan`", main= "SMA N=4 Data `Kurs Jual Rataan`", ylim=c(14100, max(data2, na.rm=TRUE)))
points(data2)
...
error_test.sma = test_ma.ts-data.gab[84:103,3]
...
```{r}
ts.plot(data2, xlab="Time `Minggu Ke-` ", ylab="`Kurs Jual Rataan`", main= "DMA N=4 Data `Kurs Jual Rataan`",ylim=c(14100, max(data2, na.rm=TRUE)))
points(data2)
...
```{r}
#Menghitung nilai keakuratan data uji
error_test.dma = test_ma.ts-data.gab2[84:103,6]
...
```{r}
# bagi data
training <- data1[313:(313+n_train), ]
testing  <- data1[(314+n_train):415, ]
...
ggplot() + 
  ...
  labs(x = "`Minggu Ke-t` Waktu", y = "Membaca", color = "Legend") +
  scale_colour_manual(name="Keterangan:", breaks = c("Data Latih", "Data Uji"),
                      values = c("blue", "red")) + 
  theme_bw() + theme(legend.position = "bottom",
                     plot.caption = element_text(hjust=0.5, size=12))
...
Single Exponential Smoothing (SES) adalah metode peramalan deret waktu yang dikembangkan untuk mengatasi kelemahan Moving Average. Jika pada Moving Average setiap data \`Minggu Ke-t\` sebelumnya dianggap memiliki bobot yang sama, maka SES memberikan bobot yang semakin mengecil secara eksponensial terhadap data lama, sehingga data terbaru diberi bobot lebih besar.
...
-   `h` : jumlah \`Minggu Ke-t\` ke depan yang ingin diramalkan (forecast horizon).

    Kasus di atas merupakan contoh inisialisasi nilai parameter $\lambda$ dengan nilai `alpha` 0,2 dan 0,7 dan banyak \`Minggu Ke-t\` data yang akan diramalkan adalah sebanyak 10 \`Minggu Ke-t\`.
...
autoplot(ses.1) +
  ...
  ylab("Membaca") + xlab("`Minggu Ke-t`")
```

Selanjutnya akan digunakan fungsi `HoltWinters()` dengan nilai inisialisasi parameter dan panjang \`Minggu Ke-t\` peramalan yang sama dengan fungsi `ses()` .
...
resid1<-training$`Kurs Jual Rataan`-ramalan1$fitted
head(resid1)
...
resid2<-training$`Kurs Jual Rataan`-ramalan2$fitted
head(resid2)
...
```{r}
accuracy(ramalan1, testing$`Kurs Jual Rataan`)
accuracy(ramalan2, testing$`Kurs Jual Rataan`)
accuracy(ramalanopt, testing$`Kurs Jual Rataan`)
```

## *Double Exponential Smoothing* (DES)

Metode pemulusan *Double Exponential Smoothing* (DES) digunakan untuk data yang memiliki pola tren. Metode DES adalah metode semacam SES, hanya saja dilakukan dua kali, yaitu pertama untuk tahapan 'level' dan kedua untuk tahapan 'tren'. Pemulusan menggunakan metode ini akan menghasilkan peramalan tidak konstan untuk \`Minggu Ke-t\` berikutnya.
...
selisihdes1<-ramalandes1$mean-testing$`Kurs Jual Rataan`
selisihdes1

SSEtestingdes1<-sum(selisihdes1^2)
MSEtestingdes1<-SSEtestingdes1/length(testing$`Kurs Jual Rataan`)
MAPEtestingdes1<-sum(abs(selisihdes1/testing$`Kurs Jual Rataan`)*100)/length(testing$`Kurs Jual Rataan`)

selisihdes2<-ramalandes2$mean-testing$`Kurs Jual Rataan`
selisihdes2

SSEtestingdes2<-sum(selisihdes2^2)
MSEtestingdes2<-SSEtestingdes2/length(testing$`Kurs Jual Rataan`)
MAPEtestingdes2<-sum(abs(selisihdes2/testing$`Kurs Jual Rataan`)*100)/length(testing$`Kurs Jual Rataan`)

selisihdesopt<-ramalandesopt$mean-testing$`Kurs Jual Rataan`
selisihdesopt

SSEtestingdesopt<-sum(selisihdesopt^2)
MSEtestingdesopt<-SSEtestingdesopt/length(testing$`Kurs Jual Rataan`)
MAPEtestingdesopt<-sum(abs(selisihdesopt/testing$`Kurs Jual Rataan`)*100)/length(testing$`Kurs Jual Rataan`)
...
data3.ts <- ts(data3$`Kurs Jual Rataan`)
```
...
1.  Setiap kelompok memilih satu data time series dengan panjang ≥ 500 \`Minggu Ke-t\`.
